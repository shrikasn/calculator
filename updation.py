from fastapi import FastAPI, HTTPException, logger, os
...
@app.put("/update/{operation_id}", response_model=CalculationResult)
async def update_operation(operation_id: int, operator: str, num1: float, num2: float):
    """
    Update an existing operation with the given `operation_id`.
    """
    try:
        # Check if the operation exists
        query = "SELECT * FROM operations WHERE id = :operation_id"
        operation = await database.fetch_one(query, values={"operation_id": operation_id})

        if operation is None:
            raise HTTPException(status_code=404, detail="Operation not found!")

        # Perform the updated calculation based on the new inputs
        if operator == "+":
            result = num1 + num2
        elif operator == "-":
            result = num1 - num2
        ...
        query = """
        UPDATE operations
        SET num1 = :num1, num2 = :num2, operator = :operator, result = :result, operation_date = :operation_date
        ...
        logger.error(f"Error during operation update: {e}")
        raise HTTPException(status_code=500, detail="Operation update failed due to an internal error.")
